...
function deliver(output, release, content, file, relative) {
  if (!release) {
  ...
  if(relative) {
    relative = pth.join('/', relative);
    release = pth.relative(relative, release);
  }
  var target;
...
module.exports = function(options, modified, total, next) {
  ...
  modified.forEach(function(file) {
    deliver(to, file.getHashRelease(), file.getContent(), file, options.relative);
  });
